				...
				<div class="flex min-h-full items-center justify-center p-4 text-center">
					<TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
						...
						<DialogPanel
							...
							<DialogTitle as="h2" class="text-3xl text-start font-black leading-[160%] tracking-[-0.69px]">
								Publique um anúncio
							</DialogTitle>

							<form @submit.prevent="handleSubmit" class="flex flex-col gap-4">
								<InputField label="Qual o game?" input-class="w-full"
									input-placeholder="Selecione o game que deseja jogar" required>
										...
										<div class="relative mt-1">
											<div>
												...
												<ComboboxButton class="absolute inset-y-0 right-0 flex items-center">
													<PhCaretDown size="24" class="text-zinc-400 mr-4" aria-hidden="true" />
												</ComboboxButton>
											</div>
											<TransitionRoot leave="transition ease-in duration-100" leaveFrom="opacity-100"
												leaveTo="opacity-0" @after-leave="gameTextQuery = ''">
												<ComboboxOptions
													...
													<ComboboxOption v-for="game in filteredGames" as="template" :key="game.id" :value="game.id"
														v-slot="{ selected }">
														<li class="relative cursor-pointer select-none px-3 py-4 text-white hover:bg-zinc-800"
															:class="{
								...
								<InputField v-model.trim="nameOrNickname" label="Seu nome (ou nickname)"
									input-placeholder="Como te chamam dentro do game?" input-class="w-full" required />

								<div class="flex gap-6">
									<InputField v-model.number="yearsPlaying" input-type="number" label="Joga há quantos anos?"
										input-placeholder="Tudo bem ser ZERO" input-class="w-full" required />

									<InputField v-model.trim="discordName" label="Qual seu Discord?" input-placeholder="Usuario#0000"
										input-class="w-full" required />
								</div>

								<div class="flex flex-col justify-center items-start gap-2">
									<span class="text-white text-base font-semibold leading-[26px] tracking-[-0.18px]">
										Quando costuma jogar?
									</span>

									<div class="flex w-full justify-between items-center">
										<label v-for="day in weekDaysOptions" :key="`week-day-checkbox:${day.value}`"
											:for="`week-day-checkbox:${day.value}`" class="block relative w-10 h-10 cursor-pointer"
											:title="day.name">
											<input type="checkbox" v-model="weekDaysPlaying"
												class="appearance-none w-full h-full rounded-md bg-zinc-900 checked:bg-violet-500 text-white cursor-pointer"
												:value="day.value" :text="day.text" :id="`week-day-checkbox:${day.value}`"
								...
								<div class="flex gap-4">
									<InputField label="A partir de qual horário?" class="flex-1" v-slot="{ id }">
										<TimeInput v-model="hourStart" :id="id" class="w-full" required />
									</InputField>

									<InputField label="Até qual horário?" class="flex-1" v-slot="{ id }">
										<TimeInput v-model="hourEnd" :id="id" class="w-full" required />
									</InputField>
								</div>

								<label for="use-voice-channel-checkbox"
									class="flex gap-2 cursor-pointer mt-2 text-sm text-white font-normal leading-[26px] tracking-[-0.09px] select-none">
									<div class="relative w-6 h-6">
										<input type="checkbox" v-model="useVoiceChannel" id="use-voice-channel-checkbox"
											class="appearance-none w-full h-full bg-zinc-900 rounded-[0.25rem] cursor-pointer" />
										<PhCheck v-show="useVoiceChannel" size="16"
